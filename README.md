...
local function getAllPlayers()
    ...
    for _, player in pairs(game:GetService("Players"):GetPlayers()) do
        table.insert(players, player.Name) -- Adiciona o nome do jogador à lista
    end
    return players
end

-- Função para atualizar o Dropdown dinamicamente
local function refreshDropdown(dropdown)
    if dropdown then
        local players = getAllPlayers()
        dropdown:Refresh(players, true) -- Atualiza as opções do dropdown
    end
end

-- Criação do Dropdown para Force Carry
local ForceCarryDropdown = Tabmisc:CreateDropdown({
    Name = "🧸 Force Carry",
    Options = getAllPlayers(), -- Popula o dropdown com a lista inicial de jogadores
    CurrentOption = nil, -- Nenhum jogador selecionado por padrão
        ...
        if selectedPlayer then
            ...
            if targetPlayer and player.Character then
                ...
                if targetHumanoidRootPart then
                    ...
                    -- Caso não encontre o HumanoidRootPart (ou o personagem do alvo não tenha sido carregado)
                    Rayfield:Notify({
                        Title = "Erro",
                        Content = "Não foi possível teletransportar. O jogador pode estar em uma área não carregada.",
                        Duration = 5,
                        Image = "error",
                        Actions = {
                            Confirm = {
                                Name = "Okay",
                                Callback = function()
                                    print("Erro ao teletransportar para o jogador selecionado.")
                                end
                            }
                        }
                    })
                ...
                local ohInstance1 = game:GetService("Players")[selectedPlayer] -- Acessando o jogador selecionado
                ...
                Rayfield:Notify({
                    Title = "Force Carry",
                    Content = "Teleported to and requested force carry from " .. selectedPlayer .. ".",
                    Duration = 5,
                    Image = "users",
                    Actions = {
                        Confirm = {
...
-- Atualiza o dropdown dinamicamente quando jogadores entram ou saem
game.Players.PlayerAdded:Connect(function()
    refreshDropdown(ForceCarryDropdown)
end)
game.Players.PlayerRemoving:Connect(function()
    refreshDropdown(ForceCarryDropdown)
end)

-- Atualiza o dropdown ao inicializar
refreshDropdown(ForceCarryDropdown)
